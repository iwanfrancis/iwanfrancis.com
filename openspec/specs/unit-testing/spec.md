...
Defines the project's unit-testing capability: the Vitest runner split into node and jsdom
projects (routed by test-file extension), the colocated test layout with shared test code in
`src/testing/`, the documented test conventions (Arrange/Act/Assert structure and the Kent C Dodds
React Testing Library rules, distilled into CLAUDE.md), and the required backfill coverage across
security boundaries, algorithmic invariants, route handlers/middleware, and interactive
components. Backfilled tests characterise current behaviour — suspected bugs are surfaced, not
silently fixed. E2e testing, async RSC page rendering, and coverage thresholds are out of scope.
...
`fireEvent` (sole carve-out: an event user-event cannot produce, used with a comment), `findBy*`
for asynchronous appearance, `queryBy*` only for asserting absence, `waitFor` hygiene (single
assertion, no side-effects, never empty), jest-dom matchers over truthiness, and no manual
`cleanup`, no `act()` wrapping, and no adding ARIA attributes solely to satisfy a query.
CLAUDE.md's commands table SHALL list the new test and typecheck scripts.
