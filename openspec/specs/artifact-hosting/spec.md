# artifact-hosting Specification

## Purpose

Defines how hosted Claude artifacts are stored and served publicly at
`artifacts.iwans.space`: the object-storage layout (a `<slug>/` key prefix holding
`index.html`, any assets, and a `meta.json`), how a request path maps to an object key (with
`index.html` defaulting for a slug root), content-type passthrough, immutable caching,
