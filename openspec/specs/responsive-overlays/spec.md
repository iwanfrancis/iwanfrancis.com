# responsive-overlays Specification

## Purpose

Defines how the app's shared modal overlays (`Dialog` and `AlertDialog`) present across
viewports: a centred dialog at the `sm` breakpoint and above, and a bottom-anchored sheet
below it, both drawn from one shared class recipe so they can't drift. The non-destructive
`Dialog` additionally gains a grabber and swipe-down-to-dismiss (layered on the existing
Radix component with `@use-gesture`, no root swap and no new primitive library), while the
destructive `AlertDialog` takes the sheet styling only — no grabber, no drag — so its choice
stays deliberate. Covers the viewport-appropriate presentation, the pure distance-plus-velocity
dismissal decision, reduced-motion handling, and the preservation of the standard overlay
affordances (focus trap, Escape, overlay-click, close control). Desktop presentation is
unchanged from before the capability was introduced.
## Requirements
